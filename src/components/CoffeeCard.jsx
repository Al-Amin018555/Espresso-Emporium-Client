...
const CoffeeCard = ({ coffee, coffees, setCoffees }) => {
  ...
  const handleDelete = (_id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://espresso-emporium-server-nine-delta.vercel.app/coffees/${_id}`, {
          method: "DELETE",
        })
          .then(res => res.json())
          .then(data => {
  ...
  return (
    <div>
      <div className="card card-side max-h-60 flex flex-col gap-6 md:flex-row bg-[#F5F4F1] pr-6 pl-3 py-6 shadow-sm">
        <figure>
          <img className="w-44 md:w-2xs"
            src={photo}
            alt={name} />
        </figure>
